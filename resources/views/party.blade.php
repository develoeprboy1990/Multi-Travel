...
<div class="col-md-4">
<select name="PartyCategoryID" id="PartyCategoryID" class="form-select" >
  <option value="">Select</option>
  <?php foreach ($party_category as $key => $value): ?>
    <option value="{{$value->PartyCategoryID}}" {{($value->PartyCategoryID== old('PartyCategoryID')) ? 'selected=selected':'' }}>{{$value->CategoryCode}}-{{$value->CategoryName}}</option>
  <?php endforeach ?>
</select>
</div>
</div>
