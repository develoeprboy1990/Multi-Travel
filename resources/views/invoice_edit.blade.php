...
<select name="PartyID" id="PartyID" class="form-select select2 mt-5" name="PartyID" required="">
 <?php foreach ($customer as $key => $value): ?>
     <option value="{{$value->PartyID}}" {{($value->PartyID== $invoice_mst[0]->PartyID) ? 'selected=selected':'' }}>{{$value->CategoryCode}} - {{$value->PartyName}}</option>
   <?php endforeach ?>
</select>
</div>
  </div>
    ...
    <div class="row">
              <div class="col-12">
                <div class="mb-1 row">
                  ...
                  <div class="col-sm-9">
                    <input type="text" id="first-name" class="form-control" name="VHNO" value="{{$invoice_mst[0]->InvoiceMasterID}}" >
                  </div>
                </div>
              </div>
              <div class="col-12">
                <div class="mb-1 row">
                  ...
                  <div class="col-sm-9">
                      <input type="text" name="Date"  autocomplete="off" class="form-control" placeholder="yyyy-mm-dd" data-date-format="yyyy-mm-dd" data-date-container="#datepicker21" data-provide="datepicker" data-date-autoclose="true" value="{{$invoice_mst[0]->Date}}">
            ...
            <?php foreach ($chartofaccount as $key => $value): ?>
              ...
              <option value="{{$value->ChartOfAccountID}}" {{($invoice_mst[0]->PaymentMode==$value->ChartOfAccountID) ? 'selected=selected':'' }}>{{$value->ChartOfAccountName}}</option>
            <?php endforeach ?>
...
<div class="col-12">
                <div class="mb-1 row">
                  ...
                  <?php foreach ($saleman as $key => $value): ?>
                    ...
                    <option value="{{$value->SalemanID}}" {{($value->SalemanID== $invoice_mst[0]->UserID) ? 'selected=selected':'' }}>{{$value->SalemanName}}</option>
                  <?php endforeach ?>
          ...
          <tbody>
            ...
            <tr class="p-3">
              ...
              <td>
                 ...
                 <select name="ItemID0[]" id="ItemID0_{{$no}}" class="form-select form-control-sm   changesNoo">
                  @foreach ($items as $key => $value) 
                    <option value="{{$value->ItemID}}|{{$value->Percentage}}" {{($value->ItemID== $value1->ItemID) ? 'selected=selected':'' }} >{{$value->ItemCode}}-{{$value->ItemName}}-{{$value->Percentage}}</option>
                  @endforeach
                 </select>
                 <input type="hidden" name="ItemID[]" id="ItemID_{{$no}}" value="{{$value1->ItemID}}">
              </td>
              <td> <select name="SupplierID[]" id="SupplierID_{{$no}}" class=" form-select changesNo select2" onchange="ajax_balance(this.value);">
                   @foreach ($party as $key => $value) 
                    <option value="{{$value->PartyID}}" {{($value->PartyID== $value1->SupplierID) ? 'selected=selected':'' }}>{{$value->CategoryCode}}-{{$value->PartyName}}</option>
                  @endforeach
                 </select>
                ...
                <td>
                  <input type="text" name="Sector[]" id="Sector_{{$no}}" class=" form-control changesNo" autocomplete="off" value="{{$value1->Sector}}" >
                </td>
              <td>
              ...
              <td>
                <input type="number" name="Taxable[]"  id="Taxable_{{$no}}" class=" form-control totalLinePrice " autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;" step="0.01" value="{{$value1->Taxable}}" >
              </td>
              <td>
              ...
              <td>
                <input type="number" name="OPVAT[]" id="OPVAT_{{$no}}" class=" form-control changesNo" autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;" step="0.01" value="{{$value1->OPVAT}}">
              </td>
              <td>
                <input type="number" name="IPVAT[]" id="IPVAT_{{$no}}" class=" form-control changesNo" autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;" step="0.01" value="{{$value1->IPVAT}}">
              </td>
              ...
              <td>
                <input type="number" name="TaxAmount[]" id="quantity_{{$no}}" class=" form-control " autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;" step="0.01" value="{{$value1->Taxable}}">
              </td>
              <td>
                <input type="number" name="Discount[]" id="discount_{{$no}}" class=" form-control changesNo" autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;" step="0.01" value="{{$value1->Discount}}">
              </td>
              <td>
                <input type="number" name="ItemTotal[]" id="total_{{$no}}" class=" form-control totalLinePrice" autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;" step="0.01" value="{{$value1->Total}}">
              </td>
            </tr>
            @endforeach
...
<span class="input-group-text bg-light">AED</span>              
          ...
          <div class="form-group mt-1">
            ...
            <div class="input-group">
<span class="input-group-text bg-light">AED</span>              
              <input type="number" class="form-control" id="amountPaid"  name="amountPaid" placeholder="Amount Paid" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;" step="0.01" value="{{$invoice_mst[0]->Paid}}"> 
            </div>
          </div>
          ...
          <div class="form-group mt-1">
            ...
            <div class="input-group">
<span class="input-group-text bg-light">AED</span>              
              <input type="number" class="form-control amountDue" name="amountDue"  id="amountDue" placeholder="Amount Due" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;" step="0.01" value="{{$invoice_mst[0]->Balance}}"> 
            </div>
          </div>
...
$(".addmore").on('click',function(){
  ...
  html += '<td><select name="SupplierID[]" id="SupplierID_'+i+'"  onchange="ajax_balance(this.value);" class=" form-select select2">@foreach ($supplier as $key => $value) <option value="{{$value->SupplierID}}">{{$value->SupplierName}}</option>@endforeach</select></td>';
  ...
  html += '<td><input type="text" name="Taxable[]" id="Taxable_'+i+'" class="form-control totalLinePrice " autocomplete="off" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;"></td>';
...
$('#quantity_'+id[1]).val( TaxResult ) ;
  $('#total_'+id[1]).val( ( parseFloat(Fare)+parseFloat(Service)+parseFloat(OPVAT)+parseFloat(IPVAT)-parseFloat(Discount) ).toFixed(2) );  
